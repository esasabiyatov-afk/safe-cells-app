...
Текущая версия — 4. Переход 2→3 явно переименовывает ошибочное поле срока действия ID-карты в дату выдачи. Переход 3→4 добавляет `cell_blocks`. Обе миграции выполняются только явно, сразу для рабочей и архивной версий, после резервной копии и без фоновой миграции при запуске.
...
| `block_kind` | TEXT | Только `lost_key` или `bank`. |
| `source_contract_id` | TEXT | Обязателен для `lost_key`, чтобы получить клиента из архива; для `bank` всегда `NULL`. |
...
9. `lost_key` всегда ссылается на архивный договор этой ячейки; `bank` не содержит клиента и срока.
